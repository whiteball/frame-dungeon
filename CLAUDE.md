...
- **キー操作**: 全操作の一覧は [MANUAL.md](MANUAL.md) を参照。開発上の要点のみ — 回転はターン非消費 / カニ歩き移動はターン消費（スタン中不可）。シーンアクションは数字キー `1〜0` に左から対応（既定 `[スキル, アイテム, ステータス, 足下, セーブ]` の 5 個）。アイテム使用/投擲/装備は「アイテム」ボタン1つに集約され、一覧表示中は画面下部を専用コンテキストバー（`使用/装備/投げる/置く/説明/閉じる`）に差し替える（`ItemListController` の `'inventory'` モード、コンテキストバーは `PhaserGame.vue` の Vue 側ボタンで `selectedIndex` 直読）。番号は右上隅のバッジ表示。ボタンが 7 個以上（イベント選択肢最大11個など）だと 140px 未満に縮むため `PhaserGame.vue` の `actionPages`（貪欲分割・1ページ最大6ボタン・隠れボタンは `v-show`）でページ送りする。ナビ（次ページ＞/＜前ページ）に数字キーは割り当てず、`6〜0` キーは 1 ページ目表示中でも対応アクションを実行可能（`PageUp`/`PageDown` でページ閲覧）。`Escape` でモーダルをキャンセル（`Game.keyEsc`、空アクションのセーブダイアログは除外）。文字数目安・ページ送り仕様は [docs/architecture/gameplay.md](docs/architecture/gameplay.md)。設定は `localStorage('gameSettings')` の `swapQEandAD` / `swapSandShiftS` / `debugCommands` フラグで永続化
