...
- **データ駆動 (`base.yml` 中心)**: `public/data/*.yml` を対応する Loader クラス（`BaseLoader`、`StatsLoader`、`ItemsLoader`、`EnemyLoader`、`EffectsLoader`、`TrapsLoader`、`SkillsLoader`、`ItemModifiersLoader`、`EventsLoader`）が読み込む。全 Loader の初期化は `GameDataLoader.loadAll()`（`src/lib/GameDataLoader.ts`）に集約しており、Game シーンの `create()` から一度だけ呼ぶ。Loader はいずれも Singleton (`getInstance()`) で公開され、`Player` 等のコンシューマは直接 `getInstance()` を参照する。**`base.yml` がゲーム全体の中核設定**で、ダメージ計算式・経験値必要量・レベルアップボーナス・フロア別構成（マップサイズ・敵プール・トラップ数）を formula 文字列として保持する（`expr-eval-fork` で評価）。ハードコードされた戦闘式やレベル式は存在しない。**データセット製作者向けの各 YAML のフォーマット・キー・数式変数のリファレンスは [MANUAL_DEV.md](MANUAL_DEV.md) にまとまっている**
...
- **キー操作**: 全操作の一覧は [MANUAL.md](MANUAL.md) を参照。開発上の要点のみ — 回転はターン非消費 / カニ歩き移動はターン消費（スタン中不可）。シーンアクションは数字キー `1〜0` に左から対応（既定 `[スキル, アイテム, ステータス, 足下, セーブ]` の 5 個）。アイテム使用/投擲/装備は「アイテム」ボタン1つに集約。**一覧表示中（`inventory` / `skill` / `drop` のいずれも）は画面下部をその `listMode` 専用のコンテキストバーに差し替える**（`inventory`=`使用/装備/投げる/置く/説明/閉じる`、`skill`=`発動/説明/閉じる`、`drop`=`置く/説明/閉じる`。リスト下フッターは廃止）。コンテキストバーは `PhaserGame.vue` の `ctxButtons` computed が `listMode` で出し分け、Vue 側ボタンで `selectedIndex` を直読する。番号は右上隅のバッジ表示。ボタンが 7 個以上（イベント選択肢最大11個など）だと 140px 未満に縮むため `PhaserGame.vue` の `actionPages`（貪欲分割・1ページ最大6ボタン・隠れボタンは `v-show`）でページ送りする。ナビ（次ページ＞/＜前ページ）に数字キーは割り当てず、`6〜0` キーは 1 ページ目表示中でも対応アクションを実行可能（`PageUp`/`PageDown` でページ閲覧）。`Escape` でモーダルをキャンセル（`Game.keyEsc`、空アクションのセーブダイアログは除外）。文字数目安・ページ送り仕様は [docs/architecture/gameplay.md](docs/architecture/gameplay.md)。設定は `localStorage('gameSettings')`（UI 設定。セーブデータには保存しない）の `swapQEandAD` / `swapSandShiftS` / `debugCommands` / `closeListAfterAction`（使用・装備・スキル発動後に一覧を閉じて移動可能状態へ戻す）フラグで永続化
...
- **状態異常の行動制御 (`effects.yml` `_action`)**: `timing.onAction.target: _action` の `value` にアクションツリーを与え、行動を**強制**（force: `skip`/`attack`/`attack_self`/`move`/`use_item`/`equip`/`unequip`/`use_skill`＝入力上書き・ターン消費）または**禁止**（forbid: `not_move`/`not_skill`/`not_attack`/`not_action`＝一部入力拒否・ターン非消費）する。`value` がリストなら効果ごとの `actionIndex`（共有 `count` と独立・付与時 0・純粋読み取り）で順に選び、末尾を反復（末尾 `repeat` はサブリスト巡回。`repeat` は top-level 末尾のみ）。解決は純粋関数 `src/lib/effects/StatusActionResolver.ts`、強制実行は `src/lib/map/ForcedActionExecutor.ts`（敵は `Enemy.executeForce`）、入力判定は `Game.handlePlayerActionDirective(category)` と `ItemListController.blockByDirective` に集約。引数は起動時 `YamlCrossValidator` で検証。データ仕様は [MANUAL_DEV.md](MANUAL_DEV.md)、挙動は [items.md](docs/architecture/items.md)
...
- **`public/data/*.yml` のデータ仕様（キー・型・数式変数・バリデーション）を変えたら、データセット製作者向けマニュアル [MANUAL_DEV.md](MANUAL_DEV.md) の該当節も同コミットで更新する**。特に `src/lib/*Loader.ts` の検証ロジックや formula 評価で渡す変数を変更したときは、MANUAL_DEV.md の「ファイル別リファレンス」および「数式変数リファレンス（まとめ）」と整合させること
