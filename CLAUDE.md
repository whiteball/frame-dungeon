...
- **汎用イベントオブジェクト**: `events.yml` で定義する `EventObject`。回復ポイント・祭壇・能力依存判定・選択肢付き宝箱などを 1 フォーマットで実現。**調査専用**（C キー方向選択経由）で起動し、`flavor` を表示後に `action`（即実行）/ `random_outcome`（重み付き抽選）/ `choices`（最大 10 個の選択肢メニュー）のいずれかを実行。action 種別は `heal` / `damage` / `apply_effect` / `learn_skill` / `add_modifier` / `remove_modifier_kind` / `execute_skill` / `give_item` / `spawn_enemy` / `message` / `self_destruct`。`blocking: true`（既定）で進入禁止セル化。フロア配置は `base.yml.floors[].eventCount` + `events`。詳細は [docs/architecture/events.md](docs/architecture/events.md)
